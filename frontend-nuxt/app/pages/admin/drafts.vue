    ...
    <div class="mb-8 d-flex align-end justify-space-between text-no-wrap overflow-hidden">
      <div class="header-content">
        <div class="d-flex align-center mb-1">
          ...
          <span class="text-caption font-weight-bold text-grey-darken-1">SYSTEM KEJAR TARGET</span>
        </div>
      ...
      <v-row v-for="(item, index) in drafts" :key="item.id" class="mb-8" justify="center">
        <v-col cols="12" md="11" lg="10">
          <v-card elevation="2" class="rounded-xl overflow-hidden shadow-soft border">
            <v-row no-gutters>
              <v-col cols="12" md="6" class="bg-grey-lighten-3 relative overflow-hidden viewer-bg"
                style="height: 420px;">
                <div class="zoom-wrapper" @wheel.prevent="handleWheel($event, index)"
                  ...
                  <div class="position-absolute z-index-2 d-flex flex-column align-end ma-3" style="top:0; right:0;">
                    <v-btn icon="mdi-rotate-right" size="small" color="primary" class="mb-2" elevation="2"
                      @click="rotateImage(index)"></v-btn>
                    <v-btn icon="mdi-refresh" size="small" color="white" elevation="2"
                      @click="resetView(index)"></v-btn>
                  </div>
                  ...
                  <div class="image-container d-flex align-center justify-center" :style="{
                    ...
                    <v-img :src="getImageUrl(item.image_path)" width="100%" contain draggable="false"
                      class="ktp-image-render">
                ...
                <div class="mb-6">
                  <div class="d-flex align-center justify-space-between">
                    <div class="d-flex align-center">
                      <v-icon color="primary" size="18" class="mr-2">mdi-file-pdf-box</v-icon>
                      <span class="text-body-2 font-weight-medium text-grey-darken-1">{{ item.original_filename
                        }}</span>
                    </div>
                    <v-chip color="primary" size="x-small" variant="outlined" class="font-weight-bold">
                  ...
                  <h3 class="text-h6 font-weight-bold text-grey-darken-4">
                    <v-icon color="primary" size="22" class="mr-2">mdi-account-check</v-icon>
                ...
                <v-form @submit.prevent="save(item)">
                  <div class="mb-4">
                    <div class="d-flex justify-space-between align-center ml-1 mb-1">
                      ...
                      <v-chip :color="item.nik?.length === 16 ? 'success' : 'warning'" size="x-small" variant="flat"
                        class="font-weight-black text-white">
                        {{ item.nik?.length || 0 }} / 16
                      </v-chip>
                    </div>
                    <v-text-field v-model="item.nik" variant="outlined" density="comfortable" bg-color="grey-lighten-5"
                      prepend-inner-icon="mdi-identifier" rounded="lg" hide-details
                      @input="item.nik = item.nik.replace(/\D/g, '')" maxLength="16" />
                  </div>

                  <div class="mb-8">
                    <span class="text-caption font-weight-bold text-grey-darken-2 ml-1">Nama Lengkap</span>
                    <v-text-field v-model="item.nama" variant="outlined" density="comfortable" bg-color="grey-lighten-5"
                      prepend-inner-icon="mdi-account-outline" rounded="lg" hide-details />
                  </div>

                  <v-btn color="primary" size="x-large" block class="rounded-xl font-weight-bold py-7" elevation="3"
                    :loading="item.isSaving" :disabled="!isValid(item)" @click="save(item)">
    ...
    <v-row v-else justify="center" class="mt-10">
      <v-col cols="12" md="6" class="text-center">
        <v-card class="pa-10 rounded-xl border shadow-soft" variant="flat">
          ...
          <v-btn color="primary" prepend-icon="mdi-upload" size="large" class="rounded-lg px-8 font-weight-bold"
